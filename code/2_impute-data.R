...
saveRDS(res_wjlw[[5]], '../data/imputed/wjlw_itt.rds')

res_wjap <- impute_data(
  'wjap_k', covars, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjap[[5]], '../data/imputed/wjap_itt.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/wjlw_cace.rds')

res_wjap <- impute_data(
  'wjap_k', covars, bl_ord = F,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_wjap[[5]], '../data/imputed/wjap_cace.rds')
...
res_wjpv[[5]]$household_size_log <- res_wjpv[[5]]$household_size_log_ctr
saveRDS(res_wjpv[[5]], '../data/imputed/wjpv_cace.rds')

res_htks <- impute_data(
  'htks_k', c(covars[1:3], 'household_size_log_ctr', covars[5:11]), 
  cace = T, brn = 120000, itr = 40000
)
saveRDS(res_htks[[5]], '../data/imputed/htks_cace.rds')

res_fdigit <- impute_data(
  'fdigit_k', covars, bl_ord = F, sed = 66,
  brn = 120000, itr = 40000, cace = T
)
saveRDS(res_fdigit[[5]], '../data/imputed/fdigit_cace.rds')

res_bdigit <- impute_data(
  'bdigit_k', covars, brn = 100000, itr = 40000, cace = T
)
saveRDS(res_bdigit[[5]], '../data/imputed/bdigit_cace.rds')

res_tom <- impute_data(
  'tom_k', covars, brn = 100000, itr = 40000, cace = T
)
saveRDS(res_tom[[5]], '../data/imputed/tom_cace.rds')

res_sps <- impute_data(
  'sps_k', covars, brn = 120000, itr = 40000, cace = T
)
saveRDS(res_sps[[5]], '../data/imputed/sps_cace.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/pk4/wjlw_itt.rds')

res_wjap <- impute_data(
  'wjap_pk4', covars_pk4, bl_ord = F, brn = 100000, itr = 40000, 
  sed = 66  # <- for convergence; prob unnecessary
)
saveRDS(res_wjap[[5]], '../data/imputed/pk4/wjap_itt.rds')

res_wjpv <- impute_data(
  'wjpv_pk4', covars_pk4, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjpv[[5]], '../data/imputed/pk4/wjpv_itt.rds')

res_htks <- impute_data(
  'htks_pk4', covars_pk4, brn = 100000, itr = 40000
)
saveRDS(res_htks[[5]], '../data/imputed/pk4/htks_itt.rds')

res_fdigit <- impute_data(
  'fdigit_pk4', covars_pk4, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_fdigit[[5]], '../data/imputed/pk4/fdigit_itt.rds')

res_bdigit <- impute_data(
  'bdigit_pk4', covars_pk4, brn = 100000, itr = 40000
)
saveRDS(res_bdigit[[5]], '../data/imputed/pk4/bdigit_itt.rds')

res_tom <- impute_data(
  'tom_pk4', covars_pk4, brn = 100000, itr = 40000
)
saveRDS(res_tom[[5]], '../data/imputed/pk4/tom_itt.rds')

res_sps <- impute_data(
  'sps_pk4', covars_pk4, brn = 120000, itr = 40000
)
saveRDS(res_sps[[5]], '../data/imputed/pk4/sps_itt.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/pk4/wjlw_cace.rds')

res_wjap <- impute_data(
  'wjap_pk4', covars_pk4, bl_ord = F,
  brn = 100000, itr = 40000, cace = T, sed = 66
)
saveRDS(res_wjap[[5]], '../data/imputed/pk4/wjap_cace.rds')

res_wjpv <- impute_data(
  'wjpv_pk4', covars_pk4, bl_ord = F,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_wjpv[[5]], '../data/imputed/pk4/wjpv_cace.rds')

res_htks <- impute_data(
  'htks_pk4', covars_pk4, 
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_htks[[5]], '../data/imputed/pk4/htks_cace.rds')

res_fdigit <- impute_data(
  'fdigit_pk4', covars_pk4, bl_ord = F, sed = 66,
  brn = 120000, itr = 40000, cace = T
)
saveRDS(res_fdigit[[5]], '../data/imputed/pk4/fdigit_cace.rds')

res_bdigit <- impute_data(
  'bdigit_pk4', covars_pk4,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_bdigit[[5]], '../data/imputed/pk4/bdigit_cace.rds')

res_tom <- impute_data(
  'tom_pk4', covars_pk4, brn = 100000, itr = 40000, 
  cace = T
)
saveRDS(res_tom[[5]], '../data/imputed/pk4/tom_cace.rds')

res_sps <- impute_data(
  'sps_pk4', covars_pk4, brn = 120000, itr = 40000, 
  cace = T
)
saveRDS(res_sps[[5]], '../data/imputed/pk4/sps_cace.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/pk3/wjlw_itt.rds')

res_wjap <- impute_data(
  'wjap_pk3', covars_pk3, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjap[[5]], '../data/imputed/pk3/wjap_itt.rds')

res_wjpv <- impute_data(
  'wjpv_pk3', covars_pk3, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjpv[[5]], '../data/imputed/pk3/wjpv_itt.rds')

res_htks <- impute_data(
  'htks_pk3', covars_pk3, brn = 100000, itr = 40000
)
saveRDS(res_htks[[5]], '../data/imputed/pk3/htks_itt.rds')

res_fdigit <- impute_data(
  'fdigit_pk3', covars_pk3, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_fdigit[[5]], '../data/imputed/pk3/fdigit_itt.rds')

res_bdigit <- impute_data(
  'bdigit_pk3', covars_pk3, brn = 100000, itr = 40000
)
saveRDS(res_bdigit[[5]], '../data/imputed/pk3/bdigit_itt.rds')

res_tom <- impute_data(
  'tom_pk3', covars_pk3, brn = 100000, itr = 40000
)
saveRDS(res_tom[[5]], '../data/imputed/pk3/tom_itt.rds')

res_sps <- impute_data(
  'sps_pk3', covars_pk3, brn = 120000, itr = 40000
)
saveRDS(res_sps[[5]], '../data/imputed/pk3/sps_itt.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/pk3/wjlw_cace.rds')

res_wjap <- impute_data(
  'wjap_pk3', covars_pk3, bl_ord = F,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_wjap[[5]], '../data/imputed/pk3/wjap_cace.rds')

res_wjpv <- impute_data(
  'wjpv_pk3', covars_pk3, bl_ord = F,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_wjpv[[5]], '../data/imputed/pk3/wjpv_cace.rds')

res_htks <- impute_data(
  'htks_pk3', covars_pk3, 
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_htks[[5]], '../data/imputed/pk3/htks_cace.rds')

res_fdigit <- impute_data(
  'fdigit_pk3', covars_pk3, bl_ord = F, sed = 66,
  brn = 120000, itr = 40000, cace = T
)
saveRDS(res_fdigit[[5]], '../data/imputed/pk3/fdigit_cace.rds')

res_bdigit <- impute_data(
  'bdigit_pk3', covars_pk3,
  brn = 100000, itr = 40000, cace = T
)
saveRDS(res_bdigit[[5]], '../data/imputed/pk3/bdigit_cace.rds')

res_tom <- impute_data(
  'tom_pk3', covars_pk3, brn = 100000, itr = 40000, 
  cace = T
)
saveRDS(res_tom[[5]], '../data/imputed/pk3/tom_cace.rds')

res_sps <- impute_data(
  'sps_pk3', covars_pk3, brn = 120000, itr = 40000, 
  cace = T
)
saveRDS(res_sps[[5]], '../data/imputed/pk3/sps_cace.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/alt_covars/bl_y_only/wjlw.rds')

res_wjap <- impute_data(
  'wjap_k', covars_bl_y, bl_ord = F, brn = 50000, itr = 40000
)
saveRDS(res_wjap[[5]], '../data/imputed/alt_covars/bl_y_only/wjap.rds')

res_wjpv <- impute_data(
  'wjpv_k', covars_bl_y, bl_ord = F, brn = 50000, itr = 40000
)
res_wjpv[[5]]$household_size_log <- res_wjpv[[5]]$household_size_log_ctr
saveRDS(res_wjpv[[5]], '../data/imputed/alt_covars/bl_y_only/wjpv.rds')

res_htks <- impute_data(
  'htks_k', covars_bl_y, brn = 50000, itr = 40000
)
saveRDS(res_htks[[5]], '../data/imputed/alt_covars/bl_y_only/htks.rds')

res_fdigit <- impute_data(
  'fdigit_k', covars_bl_y, bl_ord = F, brn = 50000, itr = 40000
)
saveRDS(res_fdigit[[5]], '../data/imputed/alt_covars/bl_y_only/fdigit.rds')

res_bdigit <- impute_data(
  'bdigit_k', covars_bl_y, brn = 50000, itr = 40000
)
saveRDS(res_bdigit[[5]], '../data/imputed/alt_covars/bl_y_only/bdigit.rds')

res_tom <- impute_data(
  'tom_k', covars_bl_y, brn = 50000, itr = 40000
)
saveRDS(res_tom[[5]], '../data/imputed/alt_covars/bl_y_only/tom.rds')

res_sps <- impute_data(
  'sps_k', covars_bl_y, brn = 50000, itr = 40000
)
saveRDS(res_sps[[5]], '../data/imputed/alt_covars/bl_y_only/sps.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/alt_covars/bl_y_all/wjlw.rds')

res_wjap <- impute_data(
  'wjap_k', covars_bl_y, 
  bl_ord = F, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_wjap[[5]], '../data/imputed/alt_covars/bl_y_all/wjap.rds')

res_wjpv <- impute_data(
  'wjpv_k', covars_bl_y,
  bl_ord = F, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_wjpv[[5]], '../data/imputed/alt_covars/bl_y_all/wjpv.rds')

res_htks <- impute_data(
  'htks_k', covars_bl_y, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_htks[[5]], '../data/imputed/alt_covars/bl_y_all/htks.rds')

res_fdigit <- impute_data(
  'fdigit_k', covars_bl_y, bl_ord = F, brn = 10000, itr = 5000,
  all_bl_y = T
)
saveRDS(res_fdigit[[5]], '../data/imputed/alt_covars/bl_y_all/fdigit.rds')

res_bdigit <- impute_data(
  'bdigit_k', covars_bl_y, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_bdigit[[5]], '../data/imputed/alt_covars/bl_y_all/bdigit.rds')

res_tom <- impute_data(
  'tom_k', covars_bl_y, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_tom[[5]], '../data/imputed/alt_covars/bl_y_all/tom.rds')

res_sps <- impute_data(
  'sps_k', covars_bl_y, brn = 10000, itr = 5000, all_bl_y = T
)
saveRDS(res_sps[[5]], '../data/imputed/alt_covars/bl_y_all/sps.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/alt_covars/age_ctrl/wjlw.rds')

res_wjap <- impute_data(
  'wjap_k', covars_age, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjap[[5]], '../data/imputed/alt_covars/age_ctrl/wjap.rds')

res_wjpv <- impute_data(
  'wjpv_k', covars_age, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_wjpv[[5]], '../data/imputed/alt_covars/age_ctrl/wjpv.rds')

res_htks <- impute_data(
  'htks_k', covars_age, brn = 100000, itr = 40000
)
saveRDS(res_htks[[5]], '../data/imputed/alt_covars/age_ctrl/htks.rds')

res_fdigit <- impute_data(
  'fdigit_k', covars_age, bl_ord = F, brn = 100000, itr = 40000
)
saveRDS(res_fdigit[[5]], '../data/imputed/alt_covars/age_ctrl/fdigit.rds')

res_bdigit <- impute_data(
  'bdigit_k', covars_age, brn = 100000, itr = 40000
)
saveRDS(res_bdigit[[5]], '../data/imputed/alt_covars/age_ctrl/bdigit.rds')

res_tom <- impute_data(
  'tom_k', covars_age, brn = 100000, itr = 40000
)
saveRDS(res_tom[[5]], '../data/imputed/alt_covars/age_ctrl/tom.rds')

res_sps <- impute_data(
  'sps_k', covars_age, brn = 120000, itr = 40000
)
saveRDS(res_sps[[5]], '../data/imputed/alt_covars/age_ctrl/sps.rds')
...
saveRDS(res_wjlw[[5]], '../data/imputed/no_rank_choice/wjlw.rds')

res_wjap <- impute_data(
  'wjap_k', covars_household_ctr, 
  bl_ord = F, brn = 100000, itr = 40000, dat = df_no_rc
)
saveRDS(res_wjap[[5]], '../data/imputed/no_rank_choice/wjap.rds')

res_wjpv <- impute_data(
  'wjpv_k', covars_household_ctr, 
  bl_ord = F, brn = 100000, itr = 40000, dat = df_no_rc
)
saveRDS(res_wjpv[[5]], '../data/imputed/no_rank_choice/wjpv.rds')

res_htks <- impute_data(
  'htks_k', covars_household_ctr, 
  brn = 100000, itr = 40000, dat = df_no_rc
)
saveRDS(res_htks[[5]], '../data/imputed/no_rank_choice/htks.rds')

res_fdigit <- impute_data(
  'fdigit_k', covars_household_ctr, 
  bl_ord = F, brn = 100000, itr = 40000, dat = df_no_rc
)
saveRDS(res_fdigit[[5]], '../data/imputed/no_rank_choice/fdigit.rds')

res_bdigit <- impute_data(
  'bdigit_k', covars_household_ctr, 
  brn = 100000, itr = 40000, dat = df_no_rc
)
saveRDS(res_bdigit[[5]], '../data/imputed/no_rank_choice/bdigit.rds')

res_tom <- impute_data(
  'tom_k', covars_household_ctr, brn = 100000, itr = 40000, 
  dat = df_no_rc
)
saveRDS(res_tom[[5]], '../data/imputed/no_rank_choice/tom.rds')

res_sps <- impute_data(
  'sps_k', covars_household_ctr, brn = 120000, itr = 40000, 
  dat = df_no_rc
)
saveRDS(res_sps[[5]], '../data/imputed/no_rank_choice/sps.rds')
...
for (i in 1:length(y)) {  # Loop through pscore estimation & save data
  saveRDS(
    ...
    str_c('../data/imputed/propensity_scores/', file_name[i])
  )
}
...
for (i in 1:length(outcomes_k)) {  # Loop through pscore estimation & save data
  saveRDS(
    ...
    str_c(
      '../data/imputed/propensity_scores/', 
