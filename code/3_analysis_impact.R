...
pak::pkg_install(c('this.path', 'purrr', 'tibble', 'fixest', 'broom', 'tidyr'))
...
df_list <- list.files('../data/imputed/moderators/hispanic', full.names = T) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
...
df_list <- list.files('../data/imputed/moderators/income', full.names = T) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
...
df_list <- list.files('../data/imputed/moderators/education', full.names = T) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
func_params <- bind_rows(
  func_params,
...
df_list <- list.files('../data/imputed/moderators/gender', full.names = T) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
func_params <- bind_rows(
  func_params,
...
df_list <- list.files('../data/imputed/moderators/htks', full.names = T) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
func_params <- bind_rows(
  func_params,
...
df_list <- list.files(
  '../data/imputed/moderators/baseline_outcome', full.names = T
) |>
  set_names(~ str_remove(basename(.x), '\\.rds')) |> 
  map(readRDS)
func_params <- bind_rows(
...
df_list_pk4 <- list.files('../data/imputed/pk4', '_itt', full.names = T) |> 
  set_names(~ str_remove(basename(.x), '_itt.rds')) |> 
  map(readRDS)
df_list_pk3 <- list.files('../data/imputed/pk3', '_itt', full.names = T) |> 
  set_names(~ str_remove(basename(.x), '_itt.rds')) |> 
  map(readRDS)
df_list <- list(  # Arrange in proper order
...
df_list_pk4 <- list.files('../data/imputed/pk4', '_cace', full.names = T) |> 
  set_names(~ str_remove(basename(.x), '_cace.rds')) |> 
  map(readRDS)
df_list_pk3 <- list.files('../data/imputed/pk3', '_cace', full.names = T) |> 
  set_names(~ str_remove(basename(.x), '_cace.rds')) |> 
  map(readRDS)
df_list <- list(
  df_list_pk4[['wjlw']], df_list_pk4[['wjap']], df_list_pk4[['wjpv']], 
...
treat_fx <- map(res, `[[`, 2) |> 
  ...
  relocate(wave) |> relocate(y) |> relocate(p, .after = se) |> 
  relocate(g, .after = p) |> relocate(g_se, .after = g)
write.csv(treat_fx, '../output/pk/cc_cace.csv', na = '', row.names = F)
